...
Timeless is available as a portable Windows application and as a Firefox desktop extension. The Windows binary includes its own runtime and local metadata service and does not require Node.js or npm. The Firefox edition runs entirely inside Firefox and supports manual source entry; automatic DOI/ISBN/PMID/arXiv/URL metadata lookup remains desktop-only because it depends on the bundled local translation service. The source launcher requires Node.js and npm to be installed and available on `PATH`.
...
- **Add source** — manual entry across 37 bibliographic source types, with type-specific fields, creator roles, and field preservation when changing source type. The Windows/source edition also exposes automatic metadata lookup in the same form.
- **Source lookup (Windows/source edition)** — DOI, ISBN, PMID, arXiv identifiers, and URLs resolve through Timeless's bundled local metadata service; no external reference manager is required. The Firefox edition omits this lookup because extensions cannot start Timeless's bundled local Node service.
...
The Firefox edition packages the same Timeless writing, Evidence, citation, native XML, autosave, and direct PDF-export workflows as a Manifest V3 desktop extension. It requests no browsing permissions or host permissions and declares no data collection.

Because Firefox does not expose Chromium's writable file-handle API, `.timeless.xml` files open through the standard browser file picker and Save downloads a new XML copy. Browser recovery/autosave continues to use IndexedDB with a localStorage fallback.

The Firefox build deliberately omits automatic source metadata lookup. Manual source entry, CSL citation/reference generation, Evidence notes, XML round-tripping, and PDF export remain available.

To stage, lint, and package the Firefox release from source, run `npm run firefox:release`. Mozilla reviewer build instructions are under `targets/firefox/REVIEWER_BUILD.md`.
